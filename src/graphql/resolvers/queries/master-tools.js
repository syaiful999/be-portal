...
const masterToolsResolverQuery = {
  master_tools: async (_parent, { skip, take, filter, sort, must_active = true }) => {
    let query = `select ${fieldUserView} from v_master_tool `
    let countQuery = 'select count(id) as total from v_master_tool '
    if(filter) {
