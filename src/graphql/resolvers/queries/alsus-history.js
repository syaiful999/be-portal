...
const fieldAlsusView = '"id", "alsus_name","condition_type_id", "condition_type", "ownership_type_id", "ownership_type", "alsus_code", "alsus_type_id", "alsus_type_name", "rack_id", "rack_name", "acquisition_type_id", "acquisition_type", "status_id", "status_type", "employee_id", "employee_name", "equipment_pick_date", "equipment_condition", "expected_maintenance_date", "maintenance_date", "input_date", "alsus_image", "alsus_image_2", "alsus_image_3", "description", "created_date", "created_by", "modified_date", "modified_by", "is_active"'

const AlsusHistoryResolverQuery = {
  alsus_history: async (_parent, { skip, take, filter, sort, must_active = true, status, alsus_code }) => {
    let query = `select ${fieldAlsusView} from v_alsus_history `
    let countQuery = 'select count(id) as total from v_alsus_history '
    if (filter) {
...
export default AlsusHistoryResolverQuery