...
const masterWeaponsResolverQuery = {
  master_weapons: async (_parent, { skip, take, filter, sort, must_active = true }) => {
    let query = `select ${fieldUserView} from v_master_weapon `
    let countQuery = 'select count(id) as total from v_master_weapon '
    if(filter) {
...
export default masterWeaponsResolverQuery