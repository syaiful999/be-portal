...
const videoStreamingQuery = {
  cms_video_streaming: async (_parent, { skip, take, filter, sort, must_active = true }) => {
    let query = `select * from v_cms_video_streaming `
    let countQuery = 'select count(id) as total from v_cms_video_streaming '
    if(filter) {
...
export default videoStreamingQuery