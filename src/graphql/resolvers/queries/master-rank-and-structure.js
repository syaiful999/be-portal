...
const masterRankAndStructureResolverQuery = {
  master_rank_and_structures: async (_parent, { skip, take, filter, sort, must_active = true }) => {
    let query = `select ${fieldUserView} from v_master_rank_and_structure `
    let countQuery = 'select count(id) as total from v_master_rank_and_structure '
    if(filter) {
